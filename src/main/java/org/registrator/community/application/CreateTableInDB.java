...
public class CreateTableInDB {
	...
	public static void importData() {
		...
		User admin = new User("oleks", "pass1", role, "Олександр", "Архилюк",
				"Олександрович", "oless.@gmail.com", "UNBLOCK");
		DaoFactory.get().getUserDao().add(admin);
		DaoFactory
				.get()
				.getAddressDao()
				.add(new Address(admin, "79026", "Львівська", "Львів",
						"Пастернака", "35"));
		DaoFactory
				.get()
				.getPassportInfoDao()
				.add(new PassportInfo(admin, "КС", 2234,
						"Published by...data ..."));
		...
		DaoFactory
				.get()
				.getAddressDao()
				.add(new Address(registrator, "29000", "Хмельницька", null,
		...
		User user = new User("ivan", "pass3", role, "Юрій", "Іванов",
				"Іванович", "ivan.@gmail.com", "UNBLOCK");
		DaoFactory.get().getUserDao().add(user);
		DaoFactory
				.get()
				.getAddressDao()
				.add(new Address(user, "353567", "Львівська", "Стрийський",
						"Стрий", "Героїв Майдану", "30", "0"));
		DaoFactory
				.get()
				.getPassportInfoDao()
				.add(new PassportInfo(user, "КК", 123456,
		...
		LineSize bandwidth = new LineSize(radiofrequency, "cмуга радіочастот",
				"МГц");
		DiscreteValue power = new DiscreteValue(radiofrequency, "потужність",
				"мВт");
		DiscreteValue tension = new DiscreteValue(radiofrequency,
				"напруженість", "мВт");
		...
		DaoFactory.get().getAreaDao()
				.add(new Area(forest, 1, 49.86378, 24.02591));
		DaoFactory.get().getAreaDao()
				.add(new Area(forest, 2, 49.86372, 24.02599));
		DaoFactory.get().getAreaDao()
				.add(new Area(forest, 3, 49.86362, 25.02599));
		DaoFactory.get().getAreaDao()
				.add(new Area(forest, 4, 50.86362, 24.07699));
		DaoFactory.get().getStoreOfDiscreteValuesDao()
				.add(new StoreOfDiscreteValues(forest, perimeter, 0.0868));
		DaoFactory.get().getStoreOfDiscreteValuesDao()
				.add(new StoreOfDiscreteValues(forest, square, 127.7));

		Resource radiofreq = new Resource(radiofrequency, "123555",
				"радіочастоти", registrator, new Date(), "ACTIVE", tome,
				"Паспорт громадянина україни...");
		DaoFactory.get().getResourceDao().add(radiofreq);

		DaoFactory.get().getAreaDao()
				.add(new Area(radiofreq, 1, 53.876, 30.01));
		DaoFactory.get().getAreaDao().add(new Area(radiofreq, 2, 63.55, 33.76));
		DaoFactory.get().getAreaDao()
				.add(new Area(radiofreq, 3, 49.3552, 43.54));
		DaoFactory.get().getAreaDao()
				.add(new Area(radiofreq, 4, 50.345, 24.07699));
		DaoFactory.get().getStoreOfLineSizesDao()
				.add(new StoreOfLineSizes(radiofreq, bandwidth, 2400d, 2483.5));
		DaoFactory.get().getStoreOfLineSizesDao()
				.add(new StoreOfLineSizes(radiofreq, bandwidth, 5150d, 5350d));
		DaoFactory.get().getStoreOfLineSizesDao()
				.add(new StoreOfLineSizes(radiofreq, bandwidth, 2500d, 2700d));

		DaoFactory.get().getStoreOfDiscreteValuesDao()
				.add(new StoreOfDiscreteValues(radiofreq, power, 100d));
		DaoFactory.get().getStoreOfDiscreteValuesDao()
				.add(new StoreOfDiscreteValues(radiofreq, power, 500.55));
		DaoFactory.get().getStoreOfDiscreteValuesDao()
