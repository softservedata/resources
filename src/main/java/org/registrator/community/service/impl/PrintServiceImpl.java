...
public class PrintServiceImpl implements PrintService {
	...
	private BaseFont timesNewRomanBase;
	private Font fontTimesNewRomanBase;
	private BaseFont timesNewRomanFat;
	private Font fontTimesNewRomanFat;
	private BaseFont timesNewRomanFat_Italic;
	private Font fontTimesNewRomanFat_Italic;
	private BaseFont timesNewRomanItalic;
	private Font fontTimesNewRomanItalic;

	public PrintServiceImpl() {
		try {
			timesNewRomanBase = BaseFont.createFont("Times New Roman/TIMES.TTF", BaseFont.IDENTITY_H,
					BaseFont.EMBEDDED);
			fontTimesNewRomanBase = new Font(timesNewRomanBase);
			timesNewRomanFat = BaseFont.createFont("Times New Roman/TIMESBD.TTF", BaseFont.IDENTITY_H,
					BaseFont.EMBEDDED);
			fontTimesNewRomanFat = new Font(timesNewRomanBase, 14, Font.BOLD);
			timesNewRomanFat_Italic = BaseFont.createFont("Times New Roman/TIMESBI.TTF", BaseFont.IDENTITY_H,
					BaseFont.EMBEDDED);
			fontTimesNewRomanFat_Italic = new Font(timesNewRomanBase, 9, Font.BOLDITALIC);
			timesNewRomanItalic = BaseFont.createFont("Times New Roman/TIMESI.TTF", BaseFont.IDENTITY_H,
					BaseFont.EMBEDDED);
	...
	private MandatToExtractDTO createMandatToExractDto(int inquiryId) {
		...
		MandatToExtractDTO extract = new MandatToExtractDTO();
	...
	private HeaderDTO createMandateToExtractHeader() {
	...
	private Document createMandatToExtractPdf(MandatToExtractDTO extract, HeaderDTO header)
			throws IOException, DocumentException {

		Document document = new Document(PageSize.A4, 38f, 38f, 38f, 35f);
		FileOutputStream fis = new FileOutputStream(extract.getDestinationOfFile());
		...
		Paragraph header4 = new Paragraph(
				extract.getLastNameOfUser() + " " + extract.getFirstNameOfUser() + " " + extract.getMiddleNameOfUser(),
				fontTimesNewRomanFat);
		...
		Paragraph header8 = new Paragraph(extract.getRegistratorLastName() + " " + extract.getRegistratorFirtstName()
		...
		Paragraph header9 = new Paragraph("(вул. " + extract.getRegistratorStreetName()
				...
				+ extract.getRegistratorCity() + ", " + extract.getRegistratorZipcode() + ")", fontTimesNewRomanBase);
	...
	private HeaderDTO createExtractHeader() {
		HeaderDTO header = new HeaderDTO();
	...
	private Document createExtractPdf(ExtractDTO extract, HeaderDTO header) throws IOException, DocumentException {

		Document document = new Document(PageSize.A4, 38f, 38f, 38f, 35f);
		FileOutputStream fis = new FileOutputStream(extract.getDestination());
		...
		header4.add(
				new Chunk("№" + extract.getCurrentDay() + extract.getCurrentMonthNumber() + extract.getCurrentYear() + "/"+extract.getResourceId()));
		document.add(header4);
		...
		Paragraph header8 = new Paragraph("Відомості надаються на підставі пункту 4 Положення про "
				...
				+ extract.getResourceId() + " від " + extract.getRequestDay() + "." + extract.getRequestMonthNumber() + "."
				+ "20" + extract.getRequestYear() , fontTimesNewRomanBase);
		header8.getFont().setSize(12f);
	...
	public PdfPTable createExtractTable(ExtractDTO extract) {
		...
		td = new Paragraph(extract.getTotalSquareOfObject(), fontTimesNewRomanItalic);
		cell1 = new PdfPCell(th);
		cell2 = new PdfPCell(td);
		table.addCell(cell1);
		table.addCell(cell2);
		...
		td = new Paragraph(extract.getObjectPerimetr(), fontTimesNewRomanItalic);
		cell1 = new PdfPCell(th);
		cell2 = new PdfPCell(td);
		table.addCell(cell1);
		table.addCell(cell2);
		...
		td = new Paragraph(extract.getReasonForInclusion(), fontTimesNewRomanItalic);
		cell1 = new PdfPCell(th);
		cell2 = new PdfPCell(td);
		table.addCell(cell1);
		table.addCell(cell2);
		...
		td = new Paragraph(extract.getRegistratorLastName() + " " + extract.getRegistratorFirtstName() + " "
				...
				+ " "+extract.getRegistratorStreetNumber()+", кв. "+extract.getRegistratorHomeNumber(), fontTimesNewRomanItalic);
		...
		td = new Paragraph(
				extract.getRequestDay() + "." + extract.getRequestMonthNumber() + ".20" + extract.getRequestYear() + " року",
	...
	private HeaderDTO createMandateToInputHeader() {
		HeaderDTO header = new HeaderDTO();
	...
	private Document createInputPdf(InputDTO extract, HeaderDTO header) throws IOException, DocumentException {
		...
		Paragraph header5 = new Paragraph("Адреса для листування: " + extract.getRegistratorZipcode() + " , "
				...
				+ extract.getRegistratorStreetNumber() + "/" + extract.getRegistratorHomeNumber(), fontTimesNewRomanBase);
		header5.getFont().setSize(10f);
		document.add(header5);
		...
		Paragraph header11 = new Paragraph("про про внесення відомостей до  Децентралізованого", fontTimesNewRomanBase);
		header11.setAlignment(Element.ALIGN_CENTER);
		header11.getFont().setSize(12f);
		document.add(header11);
		...
		Paragraph header13 = new Paragraph("Я, людина " + extract.getLastNameOfUser() + " "
				+ extract.getFirstNameOfUser() + " " + extract.getMiddleNameOfUser()
				+ " доручаю внести до ДМРПРУ такі відомості:"
	...
	public Document printProcurationOnSubmitInfo(Integer inquiryId) {
		...
		if (inquiry.getInquiryType().equals(InquiryType.INPUT)) {
			try {
				document=createInputPdf(createInputDto(inquiryId), createExtractHeader());
