(function (window, document, $, undefined) {
    ...
    var defaults = {
        ...
        onSuccess: function () { 
            $('input.user-text').css('border', '');
            $('#submit').removeAttr('disabled');
        },
        onFailure: function () { 
            $('input.user-text').css('border', '1px solid red');
        }
    };
    ...
    var CAPTCHA = function (config) {
        ...
        //canvasWrapper = $('<canvas>').on('click', that.generate());
        //canvasWrapper = this._container.on('click', function () { that.generate()};);
        ...
        this._button = $('<button>').text('Submit code')
                        .addClass('validate')
