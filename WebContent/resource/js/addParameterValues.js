...
function addDiscreteValue(theId, discreteValue, discreteComment) {
  ...
  var newElem = $('#discreteInput_' + theId + '_' + num).clone().attr('id', 'discreteInput_' + theId + '_' + newNum);
  newElem.children('label').text('');
  newElem.children("input:hidden").remove();
  newElem.find('input').each(function() {
    $(this).removeAttr('value');
    $(this).attr("name", $(this).attr("name").replace('valueDiscretes[' + prevNum + ']', 'valueDiscretes[' + num + ']'));
...
function addLinearValue(theId, linearBegin, linearEnd) {
  ...
  var prevNum = new Number(num - 1);
  var newNum = new Number(num + 1);
  var newElem = $('#linearInput_' + theId + '_' + num).clone().attr('id', 'linearInput_' + theId + '_' + newNum);
  newElem.children('label').text('');
  newElem.children("input:hidden").remove();
  newElem.find('input').each(function() {
    $(this).removeAttr('value');
    $(this).attr("name", $(this).attr("name").replace('segments[' + prevNum + ']', 'segments[' + num + ']'));
