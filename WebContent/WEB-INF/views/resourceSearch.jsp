    ...
    <c:forEach items="${Resources}" var="resource">
      <tr <c:if test="${resource.status}"/>>
        ...
        <td><fmt:formatDate pattern="dd-MM-yyyy"
            value="${resource.date}" /></td>
        <c:forEach items="${resource.resourceDiscrete}"
          var="resourceDiscrete">
          <td><c:forEach items="${resourceDiscrete.valueDiscretes}"
              var="valueDiscrete">
                                ${valueDiscrete.value} <br />
            </c:forEach></td>
        </c:forEach>
        <c:forEach items="${resource.resourceLinear}"
          var="resourceLinear">
          <td><c:forEach items="${resourceLinear.segments}"
              var="segment">
                            ${segment.begin} - ${segment.end} <br />
            </c:forEach></td>
        </c:forEach>
        <td><a href="get/${resource.identifier}"
          class="btn btn-success">Детальніше</a></td>
      </tr>
    </c:forEach>
  </table>
</c:if>